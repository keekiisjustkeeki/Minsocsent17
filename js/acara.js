document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== Map Rute Jalan Sehat =====
    const btnLihatRute = document.getElementById('btn-lihat-rute');
    ...
    if (btnLihatRute) {
        btnLihatRute.addEventListener('click', showMap);
    }

    if (btnCloseMap) {
        btnCloseMap.addEventListener('click', hideMap);
    }

    // Click on overlay (outside image but inside container) to close
    if (mapContainer) {
        mapContainer.addEventListener('click', function(e) {
            if (e.target === mapContainer || e.target.classList.contains('map-image')) {
                hideMap();
            }
        });
    }

    // Click on image also toggles close
    const mapImage = document.querySelector('.map-image');
    if (mapImage) {
        mapImage.addEventListener('click', hideMap);
    }
});
